...
This skill orchestrates three others: an inline mining pass (see step 1), the `skill-creator` skill (`anthropic-skills:skill-creator`, authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Locate the active workspace's transcripts before fanning out: `~/.claude/projects/<slug>/`, where `<slug>` is the workspace path with `/` replaced by `-`. Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Use the `skill-creator` skill (`anthropic-skills:skill-creator`) to author the skill. Placement:
...
- The `skill-creator` skill (`anthropic-skills:skill-creator`): skill authoring process and writing guidelines.
