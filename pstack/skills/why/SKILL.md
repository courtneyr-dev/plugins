---
name: why
description: "Use when Courtney asks 'why does X work this way', 'why did we pick Y', wants design rationale, a regression or postmortem read, or a data-backed threshold traced to its evidence. Runtime behavior goes to how."
---
...
Before spawning investigators, list the available MCP servers from the Claude Code environment: the `mcp__<server>__<tool>` entries in the tool list, plus any deferred ones surfaced by `ToolSearch`.
...
- `model`: per the pack's `~/.claude/skills/poteto-mode/references/models.md` (not this skill's `references/` folder), role why investigators
- Read-write Agent (the default). **Do not use the read-only `Explore` agent.** It has no MCP tools, which disables MCP-backed investigators entirely. The source control investigator would be safe as `Explore`, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
- `subagent_type`: `general-purpose`
- `model`: per `~/.claude/skills/poteto-mode/references/models.md`, role why synthesizer
- Read-write Agent (the default). The synthesizer's quality check spot-verifies citations, which can require MCP access. The read-only `Explore` agent has no MCP tools and defeats that.
