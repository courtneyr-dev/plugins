...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent (model per the pack's `~/.claude/skills/poteto-mode/references/models.md`, role perf-issue); review the diff. Capture a post-fix trace.
