---
name: poteto-mode
description: "Use when Courtney says poteto, /poteto-mode, or asks to work in poteto's style: playbook-driven tasks with named principles, deliberate subagents, unslopped prose, simple code, and verified work."
---
...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the **skill-creator** skill (`anthropic-skills:skill-creator`, the installed skill for authoring SKILL.md files).
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing-standard** skill (`/technical-writing-standard`).
- Before commit → the `deslop` skill from the `pro-workflow` plugin (`/pro-workflow:deslop`).
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → the matching control surface: the Claude Browser tools for browser / Electron / web UIs, running the command for CLIs and TUIs. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
**Use `subagent_type: poteto-agent` (the custom agent in `agents/poteto-agent.md`) for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers). `/poteto-mode` and `poteto-agent` route through the same wrapper. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own `subagent_type` for diverse-model review; respect what the skill prescribes, don't override to `poteto-agent`.

**Defaults for every `Agent` call.** `run_in_background: true`, file pointers not inlined context, explicit model per role (model per the pack's `~/.claude/skills/poteto-mode/references/models.md` (not this skill's `references/` folder); defaults: role code for code, roles prose and judgment for prose and judgment). Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment model (role hardest tasks) when the task needs judgment or the intent is vague, and to your strongest instruction-following model (role hardest tasks) when the work is a precisely specified sequence of steps to execute to the letter; trivial mechanical edits go to your fast code model (role code). Per-role lines in `~/.claude/skills/poteto-mode/references/models.md` override these defaults and the model choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`); a role with no line keeps its default, and a role line of `inherit-parent` or `auto` runs that role on the parent chat model (omit Agent `model`).
